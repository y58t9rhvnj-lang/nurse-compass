...
7. **様式2段階の上位教育原則は「患者理解を深める」3段階**（`compass_policy_version` **2026.4**）。7項目は確認観点（`rubric_version` **3**）。policy 変更後の既存 Export ZIP は再Exportが必要。
...
| 2026-08-31 | policy 2026.4 / rubric 3（3段階教育原則）と再Export方針を追記 |
