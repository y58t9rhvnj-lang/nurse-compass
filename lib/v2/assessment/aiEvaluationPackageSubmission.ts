 ...
 * - Form3 selected_patterns は filterForm3ForAiEvaluation で投影時のみ絞る
 * - form2_evidence_links（evidence_links）は常に空
 * - キー構造（evidence_links 含む）は維持
 * - 入力 record / form3 は mutation しない
 *
...
export function prepareAiEvaluationPackageStudentSubmission(
  ...
  let form3: AiAnonymizedAssessmentRecord["form3"] = includeForm3
    ? record.form3
    : null;
  if (form3 && isRecord(form3)) {
    const filterMode = resolveForm3AiEvalFilterMode(scope?.form3Scope);
    form3 = filterForm3ForAiEvaluation(form3, filterMode);
  }
